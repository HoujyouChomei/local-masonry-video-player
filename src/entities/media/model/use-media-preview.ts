// src/entities/media/model/use-media-preview.ts

import { useState, useEffect, useRef, RefObject } from 'react';
import { useSettingsStore } from '@/shared/stores/settings-store';
import { Media } from '@/shared/schemas/media';
import { api } from '@/shared/api';
import { isNativeVideo } from '@/shared/lib/media-extensions';
...
export const useMediaPreview = ({
  ...
  const isHeavyVideo =
    enableLargeVideoRestriction && media.size > largeVideoThreshold * 1024 * 1024;
  ...
  const canPlayNative = isNativeVideo(media.path);

  const separator = media.src.includes('?') ? '&' : '?';
  const srcUrl = shouldLoadVideo ? `${media.src}${separator}_v=${viewId}` : undefined;
  ...
  }, [inView, shouldLoadVideo, viewId, media.name, canPlayNative]);
  ...
  const handleLoadedMetadata = () => {
    ...
    if (vid.videoWidth && vid.videoHeight) {
      ...
      if (media.width !== vid.videoWidth || media.height !== vid.videoHeight) {
        shouldUpdateDB = true;
      }
    }
    ...
    if (vid.duration && !isNaN(vid.duration)) {
      ...
      if (!media.duration || Math.abs(media.duration - vid.duration) > 0.1) {
        shouldUpdateDB = true;
      }
    }
    ...
    if (shouldUpdateDB) {
      api.media.updateMetadata(media.id, vid.duration, vid.videoWidth, vid.videoHeight);
    }
  };
  ...
  const handleError = () => {
    if (inView && shouldLoadVideo) {
      logger.warn(`[Error] ${media.name} -> Retry`);
