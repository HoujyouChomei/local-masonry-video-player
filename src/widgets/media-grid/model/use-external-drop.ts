// src/widgets/media-grid/model/use-external-drop.ts
    ...
    async (e: React.DragEvent) => {
      ...
      if (files.length > 0) {
        ...
        for (const file of files) {
          const path = api.system.getFilePath(file);
          if (path && isMedia(file.name)) {
      ...
      if (url && isVideoUrl(url)) {
        ...
        try {
          const result = await api.media.download(url, folderPath);
