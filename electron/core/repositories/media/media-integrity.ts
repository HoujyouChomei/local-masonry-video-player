// electron/core/repositories/media/media-integrity.ts
...
export class MediaIntegrityRepository {
  ...
  upsertMany(toInsert: MediaCreateInput[], toUpdate: MediaUpdateInput[]): void {
  ...
  findByInode(ino: number): MediaRow[] {
    const rows = this.db
      .prepare(
        `
      SELECT * FROM media WHERE ino = ?
    `
      )
      .all(ino) as MediaRow[];
    return rows;
  ...
  findMissingCandidatesBySize(size: number): MediaRow[] {
    const rows = this.db
      .prepare(
        `
      SELECT * FROM media
    ...
    `
      ...
      .all(size) as MediaRow[];
    return rows;
  }

  updateHash(id: string, hash: string): void {
    logger.debug(`[Repo] Updating hash for ID: ${id}`);
    this.db.prepare('UPDATE media SET file_hash = ? WHERE id = ?').run(hash, id);
  }
  ...
  getImportantPaths(): string[] {
    const rows = this.db
      ...
      FROM media m
      LEFT JOIN playlist_items pi ON m.id = pi.video_id
      WHERE (m.is_favorite = 1 OR pi.playlist_id IS NOT NULL)
    `
      )
      .all() as { path: string }[];
  ...
  deleteExpiredMissingMedia(retentionPeriodDays: number): number {
    ...
    const tx = this.db.transaction(() => {
      ...
      this.db.prepare(`DELETE FROM media WHERE id IN (${idPlaceholders})`).run(...ids);
    });
