// electron/core/services/video/ffmpeg-service.ts
...
export class FFmpegService {
  ...
  public async validatePath(binaryPath: string, type: 'ffmpeg' | 'ffprobe'): Promise<boolean> {
    ...
    const cacheKey = `${binaryPath}:${type}`;
    if (FFmpegService.validationCache.has(cacheKey)) {
      return FFmpegService.validationCache.get(cacheKey)!;
    }
    ...
    if (!fileName.includes(expectedName)) {
      ...
      FFmpegService.validationCache.set(cacheKey, false);
      return false;
    }
    ...
    try {
      ...
      if (output.includes(expectedKeyword)) {
        FFmpegService.validationCache.set(cacheKey, true);
        return true;
      }
